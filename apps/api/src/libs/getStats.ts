export async function getStats(userId: number) {
    ...
    const recommendedTopicCreatorCount = await strapi.db.query('api::recommended-topic.recommended-topic').count({
      where: {
        creator: userId,
      },
    })
  
    const recommendedTweetCreatorCount = await strapi.db.query('api::recommended-tweet.recommended-tweet').count({
      where: {
        creator: userId,
      },
    })
