export default {
  async afterCreate({ result }) {
    ...
    const populatedFeedback = await strapi.entityService.findOne('api::feedback.feedback', result.id, {
        populate: ['art.artist'],
      },
    )
    ...
    if (populatedFeedback.art?.artist) {
      ...
      strapi.plugins['email'].services.email.send({
        ...
        subject: `Dear ${artist.name || artist.username} your art "${art.title}" has been ${result.status}`,
        html: `<div>
