...
export async function POST(req: Request) {
  ...
  const { prompt, numberOfPosts, charLimit } = await req.json();
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Given the following article, generate ${numberOfPosts > 0 ? numberOfPosts : 'two'} Turkish posts for Twitter. The post shouldn't include any hashtags, and shouldn't exceed ${charLimit < 200 ? charLimit : 200} characters.
